...
- **Login Security & Account Lockout**: Both `users` (staff) and `companies` (clients) tables have `loginAttempts` (integer), `isLocked` (boolean), and `lastLoginAttempt` (timestamp) fields. After 3 consecutive wrong passwords, the account is automatically locked (HTTP 423 returned). Each failed attempt shows the counter "(X/3 tentativas)". On lockout, security alerts (ACCOUNT_LOCKED) are logged for all admin/director/developer users. Admin, Director, and Developer roles can unlock accounts via `POST /api/admin/users/:id/unlock` and `POST /api/admin/companies/:id/unlock`.
- **Security Logs**: Dedicated `GET /api/security-logs` endpoint (admin-only) returns filtered security events from `systemLogs`: LOGIN, LOGIN_FAILED, LOGIN_BLOCKED, ACCOUNT_LOCKED, ACCOUNT_UNLOCKED, FRONTEND_RUNTIME_ERROR. Locked accounts summary available via `GET /api/security/locked-accounts`.
- **Saúde do Sistema Page** (`/admin/system-health`): Admin panel with 3 tabs — (1) Saúde: live health checks (DB, auth, server, response time) + failover status; (2) Logs de Segurança: filterable table of all security events; (3) Contas Bloqueadas: list of locked user/company accounts with one-click unlock button. Auto-refreshes every 30s.
- **Global Frontend Error Handler**: `window.addEventListener('error')` and `window.addEventListener('unhandledrejection')` in `main.tsx` report uncaught JS errors and unhandled promise rejections to `POST /api/logs` (action=FRONTEND_RUNTIME_ERROR) for visibility in the security logs panel.
- **Simulação de Escopo Comercial** (`/admin/scope-simulations`): Full commercial scope simulation module for B2B prospect management. Features: (1) Left panel — filterable/searchable list of simulations with status badges, quick duplicate, and summary stats; (2) Editor tabs: "Empresa" (prospect info, commercial model, billing limits, route config), "Escopo" (dynamic product scope builder with per-day grouping, real-time weekly/monthly totals, inline quantity/price editing), "Análise" (KPI cards, validation checks vs billing/route minimums, margin simulation with bar chart, client classification by weekly value, category mix breakdown, and presentation summary); (3) Convert to Client dialog — creates a Company record + ContractScope items from the simulation, sets status to "converted". Schema: `scope_simulations` table with JSONB `items` field. API: CRUD at `/api/scope-simulations` + `POST /api/scope-simulations/:id/convert`. Roles: ADMIN, DIRECTOR, DEVELOPER, OPERATIONS_MANAGER.
